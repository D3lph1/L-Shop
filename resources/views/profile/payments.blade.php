                    ...
                    <table class="table">
                        ...
                        @foreach($payments as $payment)
                            <tr @if($payment->isCompleted()) class="table-success" @endif>
                                <td>{{ $payment->getId() }}</td>
                                <td>@if(count($payment->getProducts())) @lang('content.profile.payments.table.shopping') @else @lang('content.profile.payments.table.fillupbalance') @endif</td>
                                <td>@if(count($payment->getProducts()))<a class="btn btn-info btn-sm profile-payments-info" data-url="{{ route('profile.payments.info', ['server' => $payment->getServerId(), 'payment' => $payment->getId()]) }}">@lang('content.profile.payments.table.more')</a> @endif</td>
                                ...
                                <td>@if(!$payment->isCompleted()) <a href="{{ route('payment.methods', ['server' => $payment->server_id, 'payment' => $payment->getId()]) }}" class="btn green btn-sm">@lang('content.profile.payments.table.pay')</a> @endif</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
