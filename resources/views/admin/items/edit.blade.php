                ...
                <div class="col-lg-8 offset-lg-2 col-md-10 offset-md-1 col-12 offset-xs-0">
                    <div class="row">
                            ...
                            <div class="plus-category text-center mt-1 mb-3">
                                <div class="btn-group" data-toggle="buttons">
                                    <label class="btn btn-info @if($item->type == 'item') active @endif">
                                        <input type="radio" name="item_type" id="item-set-item-type" autocomplete="off" value="item" @if($item->type == 'item') checked @endif> @lang('content.admin.items.add.type.item')
                                    </label>
                                    <label class="btn btn-info @if($item->type == 'permgroup') active @endif">
                                        <input type="radio" name="item_type" id="item-set-permgroup-type" autocomplete="off" value="permgroup" @if($item->type == 'permgroup') checked @endif> @lang('content.admin.items.add.type.perm')
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="plus-category text-center mt-1 mb-3">
                                <div class="btn-group" data-toggle="buttons">
                                    ...
                                    <label class="btn btn-info @if(!$item->image) active @endif">
                                        <input type="radio" name="image_mode" id="item-set-default-image" autocomplete="off" value="default" @if(!$item->image) checked @endif> @lang('content.admin.items.add.image.default')
                                    </label>
                        ...
                        <div class="col-sm-6 offset-sm-3 col-12 save-and-del text-center">
                            ...
                            <a href="{{ route('admin.items.edit.remove', ['server' => $currentServer->id, 'item' => $item->id]) }}" class="btn danger-color"><i class="fa fa-times fa-left"></i>@lang('content.admin.items.edit.remove')</a>
                        </div>
                    </div>
                </div>
            </div>
