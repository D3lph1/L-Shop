    ...
    <v-card hover class="shop-product">
        ...
        <v-card-title class="product-price py-2 px-3">
            <div class="mb-0">
                <v-text-field
                        ...
                        :prefix="amountLabel()"
                        :value="amount"
                        :disabled="(isPermgroup && stack === 0) || isRegionOwner || isRegionMember || isCommand"
                        v-model="amount"
                        @blur="recount"
                ></v-text-field>
            </div>
        ...
        <v-card-actions class="product-footer">
            ...
            <v-btn
                    ...
                    outline
                    color="red"
                    :disabled="amount <= stack || buttonDisabled"
                    @click="decrement"
            >
...
<script>
    export default {
        ...
        computed: {
            buttonDisabled() {
                return this.isRegionOwner || this.isRegionMember || this.isCommand;
            }
        },
        methods: {
            /**
